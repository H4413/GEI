...
En cas d'anomalie, le système doit toujours prévenir le serveur central (de façon prioritaire) afin de permettre planifier l'intervention.
\begin{itemize}

\item Exigence Non Fonctionnel: Fiabilité, Autonomie, Maintenabilité
\end{itemize}
...
Par example faire modifications des paramètres pour les différents capteurs(seuils), ou la mise mise à jour à des donnes à travers de commandes depuis le serveur central, ou client movil.
\begin{itemize}

\item Exigence Non Fonctionnel: Maintenabilité, Ergonomie, Fiabilité
\end{itemize}
\item  \textbf{Le Système embarqué doit fonctionner de manière autonome sans intervention humaine et cela mème en cas de problème environnemental:}\\
Le serveur central doit réceptionner les données provenant des "Capteurs" et "Système" des stations en temps réel, dans tous les cas(pannes, dépassement du niveau d'un seuil, problèmes avec un capteur, etc) l'utilisateur doit être averti pour réaliser le respective traitement.
\begin{itemize}

\item Exigence Non Fonctionnel: Autonomie, Robustesse, Fiabilité
\end{itemize}
\item  \textbf{ La consommation électrique doit être minimale pour des raisons évidentes d’autonomie:}\\
La durée de communication entre le système et les utilisateurs doit être seulement périodiquement, sur demande extérieur (par les clients mobiles ou serveur central) ou en cas de problèmes.
\begin{itemize}

\item Exigence Non Fonctionnel: Autonomie
\end{itemize}
\end{enumerate}
